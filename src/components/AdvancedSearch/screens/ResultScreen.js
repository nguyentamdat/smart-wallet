      ...
      this.state.purposesChosen.forEach(purpose => {
        this.unsubcribe = this.database
          ...
          .onSnapshot(
            ...
            error => {
              console.log("Error getting purposeResults: ", error);
            }
          );
      });
    }
          ...
          {this.state.resultsList.map(result => {
            return (
              <ListItem noIndent key={result.id}>
                ...
                <Right style={{ flex: 1 }}>
                  <Text>{result.data.date.toDate().toLocaleDateString()}</Text>
                  <Text>{result.data.date.toDate().toLocaleTimeString()}</Text>
                </Right>
              </ListItem>
            );
          })}
